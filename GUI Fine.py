...
class Garik:
    def __init__(self, master):
        ...
        self.current_text = StringVar(self.master, textwrap.fill(texts.default_status, 39))
        ...
        self.men.add_command(label="Quit", command=self.master.quit)
        ...
        self.boton = Button(self.master, text="Calcular IST", command=self.calculate_boton, bg="SkyBlue")
        ...
        self.status_canvas = Canvas(self.master, height=350, width=240, bg="snow", highlightbackground="gainsboro")
        ...
        self.place_all(250, -10, 40, self.input_radius, self.star_type_menu, self.albedo_menu, self.input_exo_orbit, self.input_star_radius,
                       self.boton)
        self.place_all(50, -10, 40, self.radius_label, self.star_type_label, self.albedo_label, self.orbit_label,
    ...
    def calculate_boton(self):
        width = 39
        jump = "\n\n"
        result_message = ""
        error = False
        try:
            exo_rad = float(self.input_radius.get())
        except ValueError:
        ...
        try:
            exo_orbit = float(self.input_exo_orbit.get()) * 149597870700
        except ValueError:
            result_message += textwrap.fill("L'òrbita de l'exoplaneta ha de ser un número vàlid!", width) + jump
            error = True
        try:
            star_rad = float(self.input_star_radius.get()) * 6.957 * 10 ** 8
        except ValueError:
    ...
    def show_text(self, event, text):
        self.now_hovering = CuadroFlotante(self, text)
    ...
    def place_all(x_position, y_position, y_variation, *widgets):
        for i in range(len(widgets)):
            if isinstance(widgets[i], Button):
                ...
                tempx = 0
                tempy = 0
            widgets[i].place(x=x_position + tempx, y=y_position + tempy + (i+1) * y_variation)
